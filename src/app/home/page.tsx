...
import { MoneyReceiptIcon } from "./components/Icons";
...
export default function Home() {
  ...
  return (
    <div className=" flex flex-col px-4 sm:px-2 min-h-screen pb-24">
      ...
      <Link
        href={"/search"}
        className="w-full rounded-full bg-brand-bg-shade/60 flex items-center p-4 sm:p-6 gap-x-4 my-8 sm:my-14"
      >
        <svg
          className="size-5"
          width="15"
      ...
      <div
        className="bg-brand-bg-blue/5 border border-brand-bg-blue/40 rounded-xl h-full flex justify-center items-center grow gap-x-2 
      "
      >
        <MoneyReceiptIcon />
        <div className="text-brand-navy-blue gap-0 flex flex-col">
          <div>Reciept</div>
          <div className="relative -top-1">History</div>
        </div>
      </div>
    </div>
  );
